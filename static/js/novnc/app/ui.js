...
const UI = {
    ...
    async start(options={}) {
        ...
        // Set up translations
        try {
            const localeDir = new URL('locale/', import.meta.url).href;
            await l10n.setup(LINGUAS, localeDir);
        } catch (err) {
        ...
        try {
            const pkgJson = new URL('../package.json', import.meta.url).href;
            let response = await fetch(pkgJson);
            if (!response.ok) {
